...
import math
renderlib = load_source('renderlib', 'modules\\renderlib.py')
world = load_source('world', 'modules\\world.py')
...
class MainWindow(pyglet.window.Window):
	def __init__(self):
		...
		#Setup window
		self.set_icon(pyglet.image.load('modules\\icon.png'))
		...
		self.block_select=(0,6,0)
		self.mouse_exclusive=True
	def on_draw(self):
		print self.world.player.x,self.world.player.y,self.world.player.z
		RenderMode3D(self)
		...
		RenderSelectBlock(self.block_select)
...
def ImportMods(c): #add exception for when no mods exist
	...
	for mod in c.mods:
		...
		for block in mod.blocks:
			#set rendering for some reason... doesnt render properly without it
			glEnable(GL_CULL_FACE)
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST)
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST)
			#add block
